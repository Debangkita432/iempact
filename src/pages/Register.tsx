...
const registrationSchema = z.object({
  ...
  college_name: z.string().trim().min(2, 'College name must be at least 2 characters').max(200, 'College name must be less than 200 characters'),
  event_name: z.string().min(1, 'Please select an event'),
  team_name: z.string().trim().max(100, 'Team name must be less than 100 characters').optional(),
  team_size: z.number().min(1, 'Team size must be at least 1').max(10, 'Team size cannot exceed 10'),
});
          ...
          items-center justify-center">
            ...
            >
              ...
              <h1 className="font-bebas text-4xl md:text-5xl mb-4">
                <span className="text-gradient-accent">Registration</span> Complete!
              </h1>
              <p className="font-poppins text-foreground/70 mb-8">
                Thank you for registering for {formData.event_name}. We've sent a confirmation to {formData.email}.
              </p>
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Register for IMPACT 2026 events. Join thousands of participants from 100+ colleges in the largest cultural fest." />
      </Helmet>
              ...
              <form onSubmit={handleSubmit} className="p-6 md:p-8 rounded-2xl bg-card/30 backdrop-blur-sm border border-border/30">
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className={`w-full px-4 py-3 rounded-lg bg-input border font-poppins text-sm focus:border-accent focus:outline-none transition-colors ${
                        errors.event_name ? 'border-primary' : 'border-border/50'
                      }`}
                    >
                    ...
                    {errors.event_name && (
                      <p className="mt-1 text-sm text-primary">{errors.event_name}</p>
                    )}
                  </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 rounded-lg bg-input border font-poppins text-sm placeholder:text-foreground/40 focus:border-accent focus:outline-none transition-colors ${
                          errors.full_name ? 'border-primary' : 'border-border/50'
                        }`}
                      />
                      {errors.full_name && (
                        <p className="mt-1 text-sm text-primary">{errors.full_name}</p>
                      )}
                    </div>
                    <div>
                      <label className="block font-poppins text-sm font-medium text-foreground/80 mb-2">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-4 py-3 rounded-lg bg-input border font-poppins text-sm placeholder:text-foreground/40 focus:border-accent focus:outline-none transition-colors ${
                          errors.college_name ? 'border-primary' : 'border-border/50'
                        }`}
                      />
                      {errors.college_name && (
                  ...
                  <div className="border-t border-border/30 pt-6">
                    <h3 className="font-bebas text-xl text-foreground/80 mb-4">Team Details (Optional)</h3>
                    <div className="grid md:grid-cols-2 gap-4">
                      <div>
                        <label className="block font-poppins text-sm font-medium text-foreground/80 mb-2">
                          Team Name
